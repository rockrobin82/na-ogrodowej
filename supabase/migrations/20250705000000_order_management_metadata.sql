-- Add order workflow metadata and extend the existing admin status RPC.
...
BEGIN
  ...
  IF v_order_status = 'shipped' AND p_status = 'cancelled' THEN
    RAISE EXCEPTION 'Shipped orders cannot be cancelled';
  END IF;

  IF p_status = 'cancelled' AND v_order_status <> 'shipped' THEN
